...
function showQuestion(){
  ...
  for (let i = 0; i < question.choices.length; i++){
    ...
    // it can also be done as given bellow ( inner Html)
    // btn.textContent = choice.label;
    btn.className = "choice-btn"

    btn.innerHTML = `<span class="choice-tag">${labels[i]}</span>${choice.label}`;

    // for adding the option at the end of the question 
    // we can do this approach but it assigned the return value to the onclick
    // btn.onclick = selectChoice;
